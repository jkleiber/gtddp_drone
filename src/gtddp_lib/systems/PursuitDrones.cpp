...
void PursuitDrones::linearize_dynamics_mm(const vector<VectorXd>& x_traj, const vector<VectorXd>& u_traj, const vector<VectorXd>& v_traj,
    ...
    for (int i=0;i<num_time_steps-1;i++){
        ...
        A_sec(1,4)=1;
        A_sec(2,5)=1;
        A_sec(3,6)=((u0)*(-(cos_psi_u*sin_phi_u*sin_theta_u) + cos_phi_u*sin_psi_u))/m;
        A_sec(3,7)=((u0)*cos_phi_u*cos_theta_u*cos_psi_u)/m;
        A_sec(3,8)=((u0)*(cos_psi_u*sin_phi_u - cos_phi_u*sin_theta_u*sin_psi_u))/m;
        A_sec(4,6)=-(((u0)*(cos_phi_u*cos_psi_u + sin_phi_u*sin_theta_u*sin_psi_u))/m);
        A_sec(4,7)=((u0)*cos_phi_u*cos_theta_u*sin_psi_u)/m;
        A_sec(4,8)=((u0)*(cos_phi_u*cos_psi_u*sin_theta_u + sin_phi_u*sin_psi_u))/m;
        A_sec(5,6)=-(((u0)*cos_theta_u*sin_phi_u)/m);
        A_sec(5,7)=-(((u0)*cos_phi_u*sin_theta_u)/m);
...
*/
        ...
        A_sec(13,16)=1;
        A_sec(14,17)=1;
        A_sec(15,18)=((v0)*(-(cos_psi_v*sin_phi_v*sin_theta_v) + cos_phi_v*sin_psi_v))/m;
        A_sec(15,19)=((v0)*cos_phi_v*cos_theta_v*cos_psi_v)/m;
        A_sec(15,20)=((v0)*(cos_psi_v*sin_phi_v - cos_phi_v*sin_theta_v*sin_psi_v))/m;
        A_sec(16,18)=-(((v0)*(cos_phi_v*cos_psi_v + sin_phi_v*sin_theta_v*sin_psi_v))/m);
        A_sec(16,19)=((v0)*cos_phi_v*cos_theta_v*sin_psi_v)/m;
        A_sec(16,20)=((v0)*(cos_phi_v*cos_psi_v*sin_theta_v + sin_phi_v*sin_psi_v))/m;
        A_sec(17,18)=-(((v0)*cos_theta_v*sin_phi_v)/m);
        A_sec(17,19)=-(((v0)*cos_phi_v*sin_theta_v)/m);
