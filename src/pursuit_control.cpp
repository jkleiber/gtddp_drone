 ...
 *
 */
PursuitControl::PursuitControl() : drone_traj()
{
 ...
 *
 */
PursuitControl::PursuitControl(ros::Publisher ctrl_sig_pub, ros::Publisher status_pub, int sim_status) : drone_traj()
{
...
void PursuitControl::logging_init()
{
    ...
    //Form the file name
    ss << filepath << "pursuit_" << timestamp << "_gt_data" << ".csv";
    ss >> filename;
    ...
    filename = "";
    ss.clear();
    ss << filepath << "pursuit_" << timestamp << "_cmd_data" << ".csv";
    ss >> filename;
 ...
 *
 */
void PursuitControl::recalculate_control_callback(const ros::TimerEvent& time_event)
{
    ...
    && !this->drone_traj.x_traj.empty())
    {
        ...
        if(this->timestep == 0 && this->drone_traj.x_traj.size() >= Constants::num_time_steps)
        {
            ...
            drones.feedforward_controls(this->cur_state, this->drone_traj);
            ++this->timestep;
        }
...
void PursuitControl::open_loop_control(const ros::TimerEvent& time_event)
{
    // Get the current time
    double cur_time = (ros::Time::now() - begin_time).toSec();
 ...
 *
 */
void PursuitControl::state_estimate_callback(const nav_msgs::Odometry::ConstPtr& odom)
{
    //Position
 ...
 *
 */
void PursuitControl::trajectory_callback(const gtddp_drone_msgs::Trajectory::ConstPtr& traj_msg)
{
    //Declare local variables
...
void PursuitControl::set_timer(ros::Timer& timer)
{
    this->ctrl_timer = timer;
}


PursuitControl::~PursuitControl()
{
    this->ground_truth_data.close();
    this->command_data.close();
}

double PursuitControl::angleWrap(double angle)
{
...
double PursuitControl::clamp(double val, double min_val, double max_val)
{
